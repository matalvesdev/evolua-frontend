...
function createSecurityHeaders(apiUrl: string | undefined): Record<string, string> {
  ...
  if (apiUrl) {
    try {
      ...
      // A validação da variável continua na camada de API; CSP permanece restritiva.
    }
  }
...
export default defineConfig(({ mode }) => {
  ...
  const securityHeaders = createSecurityHeaders(env.VITE_API_URL)
