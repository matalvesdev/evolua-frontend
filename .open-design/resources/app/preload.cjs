...
const OPEN_DESIGN_HOST_GLOBAL = '__od__';
const OPEN_DESIGN_HOST_VERSION = 1;
...
function actionFailure(reason, details) {
    return failure(reason, details);
}
function importFailure(reason) {
    return failure(reason);
}
function replaceWorkingDirFailure(reason) {
    return failure(reason);
}
function normalizeProjectReplaceWorkingDirResult(input) {
    if (!isRecord(input))
    ...
    if (input.ok !== true) {
        ...
        return failure(typeof input.reason === 'string' && input.reason.length > 0 ? input.reason : 'unknown failure', input.details);
    }
...
function normalizeProjectImportResult(input) {
    if (!isRecord(input))
...
const project = {
    pickAndImport: (init) => ipcRenderer.invoke('dialog:pick-and-import', init ?? null)
        .then(normalizeProjectImportResult)
        .catch((error) => importFailure(reasonFromError(error))),
    pickAndReplaceWorkingDir: (projectId) => ipcRenderer.invoke('dialog:pick-and-replace-working-dir', { projectId })
        .then(normalizeProjectReplaceWorkingDirResult)
        .catch((error) => replaceWorkingDirFailure(reasonFromError(error))),
};
const shell = {
...
function invokeUpdater(action, options) {
    return ipcRenderer.invoke(`od:update:${action}`, options ?? null);
}
const updater = {
    check: (options) => invokeUpdater('check', options),
    download: (options) => invokeUpdater('download', options),
    install: (options) => invokeUpdater('install', options),
    quit: async (options) => {
        try {
    ...
    status: (options) => invokeUpdater('status', options),
    subscribe: (listener) => {
...
const hostBridge = {
    ...
    pet: {
        setVisible: (visible) => ipcRenderer.send('desktop-pet:set-visible', Boolean(visible)),
    },
    updater,
};
contextBridge.exposeInMainWorld(OPEN_DESIGN_HOST_GLOBAL, hostBridge);
contextBridge.exposeInMainWorld('openDesignDesktop', {
    exportDiagnostics: () => ipcRenderer.invoke(DESKTOP_DIAGNOSTICS_IPC_CHANNEL),
});
