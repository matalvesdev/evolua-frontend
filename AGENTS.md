...
- ❌ Repetir em `GROUP BY` uma expressão SQL com interpolação Prisma (ex.: `AT TIME ZONE ${timeZone}`) — cada ocorrência vira um bind parameter diferente e o Postgres pode lançar `42803`; agrupar pela posição/alias selecionado (`GROUP BY 1`) e manter teste de regressão.
- ❌ Workflow web tentar gerar Prisma/migrar/compilar `backend-core` — o backend é um repositório separado e não existe no checkout web; deploy, migrations e readiness da API pertencem ao CI do repositório backend.
