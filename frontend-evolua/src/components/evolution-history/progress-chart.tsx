...
import type { ProgressChartProps } from '@/types/evolution-history'
...
export function ProgressChart({
  ...
  const chartData = useMemo(() => {
    ...
    const processedSnapshots = shouldGroup 
      ? chartDataFormatter.groupByWeek(snapshots)
      : snapshots
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    ...
    const snapshot = snapshots.find(s => 
      Math.abs(s.createdAt.getTime() - data.date.getTime()) < 60000
    )
    ...
    return (
      <div className="bg-white/95 backdrop-blur-sm border border-gray-200 rounded-xl p-4 shadow-lg">
        ...
        <div className="space-y-1">
          <p className="text-sm font-bold text-[#8A05BE]">
            Progresso: {tooltipData.progress}
          </p>
          ...
          <circle
            ...
            onClick={() => {
              const milestone = milestones.find(m =>
                Math.abs(m.date.getTime() - payload.date.getTime()) < 60000
              )
  ...
  return (
    <div className="w-full h-full">
      <ResponsiveContainer width="100%" height="100%" minHeight={isMobile ? 220 : 300} aspect={isMobile ? 1.2 : 2}>
        <LineChart
          data={chartData}
