  ...
  return (
    <section className="glass-card rounded-[2rem] p-8 relative overflow-hidden group border border-white">
      ...
      {onEdit && (
        <button
          onClick={onEdit}
          className="absolute top-6 right-6 z-10 p-2 rounded-xl bg-white/70 hover:bg-white text-gray-500 hover:text-[#8A05BE] transition-all shadow-sm ring-1 ring-black/5"
          title="Editar objetivo terapêutico"
        >
      ...
      <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start md:items-center">
        ...
        <div className="flex-1 space-y-3">
          ...
          <p className="text-gray-600 text-[15px] leading-relaxed max-w-4xl">{description}</p>
        </div>
      </div>
    </section>
  );
