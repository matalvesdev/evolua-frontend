...
export default function AgendamentosPage() {
  ...
  const { appointments: weekAppointments, loading } = useAppointments({
    startDate: weekStart,
    endDate: weekEnd,
    limit: 200,
  });
  useTodayAppointments();
  const { confirmAppointment, startAppointment, cancelAppointment, completeAppointment } =
  ...
  const { appointments: monthAppointments } = useAppointments({
  ...
  const upcomingEvents = useMemo(() => {
    ...
    return weekAppointments
      .filter(
        (a) => new Date(a.dateTime) >= new Date(now.getFullYear(), now.getMonth(), now.getDate())
      )
  ...
  return (
    <>
      ...
      {/* Navigation tabs */}
      <nav className="hidden md:block bg-transparent mb-6 px-6 lg:px-10">
      ...
      <div className="flex-1 overflow-hidden pb-16 md:pb-0">
        <div className="xl:hidden px-4 sm:px-6 pb-24 space-y-4 md:space-y-6">
          <div className="bg-white/70 border border-gray-200 rounded-2xl p-4 sm:p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base sm:text-lg font-bold text-gray-900">Agenda da Semana</h2>
            ...
            <div className="grid grid-cols-7 gap-1 md:gap-2">
              {weekDays.map((day, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`rounded-lg p-1.5 md:p-2 text-center ${isToday ? 'bg-[#820AD1]/10' : 'bg-gray-50'}`}
                  >
                    <p
                      className={`text-[10px] font-bold ${isToday ? 'text-[#820AD1]' : 'text-gray-400'}`}
                    >
                      {WEEKDAYS_SHORT[i]}
                    </p>
                    <p
                      className={`text-sm font-bold ${isToday ? 'text-[#820AD1]' : 'text-gray-800'}`}
                    >
                      {day.getDate()}
                    </p>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] gap-4 md:gap-5 items-start">
            <div className="bg-white/70 border border-gray-200 rounded-2xl p-4 sm:p-5 hidden md:block">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold text-gray-900">Resumo rápido</h3>
                <button
                  onClick={handleToday}
                  className="px-3 py-1.5 text-xs font-bold text-[#820AD1] bg-[#820AD1]/10 hover:bg-[#820AD1]/15 rounded-lg transition-colors"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl bg-gray-50 p-3">
                  ...
                  <p className="mt-2 text-2xl font-bold text-gray-900">{weekAppointments.length}</p>
                </div>
                <div className="rounded-xl bg-gray-50 p-3">
                  <p className="text-[11px] uppercase tracking-wide text-gray-400 font-semibold">
                    Próximos
                  </p>
                  <p className="mt-2 text-2xl font-bold text-[#820AD1]">{upcomingEvents.length}</p>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="rounded-xl border border-gray-100 p-3">
                  ...
                  <div className="relative mt-2">
                    ...
                    <input
                      type="text"
                      placeholder="Paciente ou tipo"
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="w-full pl-8 pr-3 py-2 text-sm bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#820AD1]/30 focus:border-[#820AD1]/30 placeholder:text-gray-400"
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-2">
              {upcomingEvents.length === 0 ? (
                <div className="bg-white/70 border border-gray-200 rounded-2xl p-6 text-center">
                  <span className="material-symbols-outlined text-2xl text-gray-300 mb-2 block">
                    event_busy
                  </span>
                  <p className="text-sm text-gray-500">Nenhum agendamento próximo</p>
                </div>
              ) : (
                upcomingEvents.slice(0, 20).map((apt) => {
                  const sc = getStatusConfig(apt.status);
                  return (
                    <button
                      key={apt.id}
                      onClick={() => router.push(`/dashboard/agendamentos/${apt.id}`)}
                      className={`w-full text-left bg-white/80 border border-gray-200 rounded-xl p-3 border-l-4 ${sc.border.replace('border-l-', 'border-l-')} transition-colors hover:bg-white`}
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${sc.bg} ${sc.text} shrink-0`}
                        >
                          {sc.label}
                        </span>
                      </div>
        ...
        <div className="hidden xl:flex h-[calc(100vh-140px)] max-w-[1400px] mx-auto">
          {/* ===== LEFT SIDEBAR ===== */}
                      ...
                      {count > 0 && d.isCurrentMonth && (
                        <div className="flex gap-0.5 mt-0.5">
                          {Array.from({ length: Math.min(count, 3) }).map((_, j) => (
                            <span
                              key={j}
                              className={`w-1 h-1 rounded-full ${isSelected ? 'bg-white/70' : 'bg-[#820AD1]'}`}
                            />
                          ))}
                        </div>
                      )}
            ...
            <div className="flex border-b border-gray-200/60 bg-white/20">
              ...
              {weekDays.map((day, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-[10px] font-bold tracking-wider ${isToday ? 'text-[#820AD1]' : 'text-gray-400'}`}
                    >
                      {WEEKDAYS_SHORT[i]}
                    </span>
                    <span
                        ...
                        >
                          ...
                          {height >= 60 && (
                            <div className="flex items-center gap-1 mt-1">
                              ...
                              <span className={`text-[9px] font-medium ${sc.text}`}>
                                {sc.label}
                              </span>
                            </div>
                          )}
