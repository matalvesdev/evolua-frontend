...
function getTypeInfo(t: string) {
  return TYPE_CONFIG[t] || { label: t, icon: 'description', color: 'text-gray-500' };
}
...
async function generateReportPdf(report: Report, therapistName: string): Promise<Blob> {
  ...
  y += 5;
  doc.text(
    `Data: ${formatDateShort(report.createdAt)}  |  Status: ${getStatusInfo(report.status).label}`,
    margin,
    y
  );
  y += 8;
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text('Documento gerado automaticamente — Evolua', pageW / 2, pageH - 6, {
      align: 'center',
    });
  }
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-150">
      <div className="bg-white rounded-2xl p-6 max-w-sm w-full flex flex-col gap-4 shadow-2xl">
        ...
        <div className="flex gap-3 justify-end pt-2">
          <button
            onClick={onCancel}
            className="px-4 py-2 rounded-full border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors"
          >
            Cancelar
          </button>
          <button
            onClick={onConfirm}
            className={`px-4 py-2 rounded-full text-sm font-bold text-white transition-all shadow-lg ${confirmColor}`}
          >
            {confirmLabel}
          </button>
        </div>
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-4 flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3 text-[11px] text-gray-500">
          <span>
            Paciente: <span className="font-medium text-gray-700">{report.patientName}</span>
          </span>
        ...
        {sections.length > 0 ? (
          <div className="flex flex-col gap-3">
            {sections.map((section, i) => (
              <div key={i}>
                {section.title && (
                  <p className="text-[11px] font-bold text-[#820AD1] uppercase tracking-wide mb-1">
                    {section.title}
                  </p>
                )}
                <p className="text-[11px] text-gray-600 leading-relaxed whitespace-pre-wrap line-clamp-4">
                  {section.text || '—'}
                </p>
              </div>
            ))}
      ...
      <div className="px-5 py-2 border-t border-gray-100 bg-gray-50/50">
        <p className="text-[8px] text-gray-400 text-center">
          Documento gerado automaticamente — Evolua
        </p>
      </div>
    </div>
  );
...
function SendReportModal({
  ...
  const nextApptInfo = nextAppointment
    ? formatAppointmentDateTime(nextAppointment.dateTime)
    : undefined;
  const defaultMessage = patient
    ? buildReportMessage(report, guardianName, patient.name, pdfUrl || undefined, nextApptInfo)
    : '';
  ...
  useEffect(() => {
    if (patient && !messageEdited) {
      const apptInfo = nextAppointment
        ? formatAppointmentDateTime(nextAppointment.dateTime)
        : undefined;
      setMessage(
        buildReportMessage(report, guardianName, patient.name, pdfUrl || undefined, apptInfo)
      );
    }
  ...
  const handleSend = async () => {
    ...
    // Build final message
    const finalMessage =
      pdfUrl && !message.includes(pdfUrl)
        ? message + `\n\n📎 *Acesse o relatório em PDF:*\n${pdfUrl}`
        : message;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl sm:rounded-3xl max-w-2xl w-full max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        {/* Step indicator */}
        {!sent && (
          <div className="px-5 sm:px-6 pt-4 flex flex-wrap items-center gap-3 shrink-0">
            <button
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-4 flex flex-col gap-4">
          ...
          {sent && (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              ...
              <p className="text-sm text-gray-500">
                O relatório foi enviado para o responsável via WhatsApp.
              </p>
            </div>
          )}
          ...
          {!sent && patientLoading && (
            <div className="flex items-center justify-center py-12">
              <span className="material-symbols-outlined animate-spin text-[#820AD1] text-2xl">
                progress_activity
              </span>
            </div>
          )}
          ...
          {!sent && !patientLoading && step === 'send' && (
            <>
              ...
              <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 border border-gray-100">
                <div
                  className={`w-9 h-9 rounded-xl ${typeInfo.color === 'text-[#820AD1]' ? 'bg-[#820AD1]/10' : 'bg-gray-100'} flex items-center justify-center shrink-0`}
                >
                  <span className={`material-symbols-outlined ${typeInfo.color} text-[18px]`}>
                    {typeInfo.icon}
                  </span>
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-gray-500">
                    {typeInfo.label} · {formatDate(report.createdAt)}
                  </p>
                </div>
                {uploadingPdf ? (
                  <span className="material-symbols-outlined animate-spin text-[#820AD1] text-[18px]">
                    progress_activity
                  </span>
                ) : pdfUrl ? (
              ...
              {(uploadingPdf || pdfUrl) && (
                <div
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-medium ${pdfUrl ? 'bg-green-50 text-green-700' : 'bg-purple-50 text-[#820AD1]'}`}
                >
                  {uploadingPdf ? (
                    <>
                      ...
                      Gerando e enviando PDF para a nuvem...
                    </>
                  ) : (
                    <>
                      <span className="material-symbols-outlined text-[14px]">cloud_done</span>PDF
                      gerado e hospedado — link incluído na mensagem
                    </>
                  )}
                </div>
              )}
              ...
              {guardianPhone ? (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 border border-gray-100">
                  <div className="w-9 h-9 rounded-xl bg-purple-50 flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-[#820AD1] text-[18px]">
                      person
                    </span>
                  </div>
                ...
                <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-700 mt-1">
                      Cadastre o telefone do responsável para enviar via WhatsApp.
                    </p>
                    <Link
                      href={`/dashboard/pacientes/${report.patientId}/editar`}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <p className="text-[11px] text-gray-400">
                  Mensagem personalizada para &ldquo;{typeInfo.label}&rdquo;. O link do PDF está
              ...
              {!appointmentsLoading && nextAppointment && (
                <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-purple-50 border border-purple-100">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-[#820AD1] uppercase tracking-wide">
                      Próxima sessão agendada
                    </p>
                    <p className="text-sm font-bold text-gray-900 mt-1">
                      {formatAppointmentDateTime(nextAppointment.dateTime).full}
                    </p>
                    <p className="text-[11px] text-purple-600 mt-1">
                      A mensagem inclui confirmação da devolutiva nesta sessão.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="px-5 sm:px-6 py-4 border-t border-gray-100 flex flex-col-reverse sm:flex-row gap-3 shrink-0">
            {step === 'preview' ? (
              <>
                <button
                  onClick={onClose}
                  className="flex-1 h-11 rounded-xl border border-gray-200 text-gray-700 hover:bg-gray-50 font-bold text-sm flex items-center justify-center transition-all"
                >
                  Cancelar
                </button>
                <button
                ...
                >
                  {isBusy ? (
                    <>
                      ...
                      {uploadingPdf ? 'Preparando PDF...' : 'Enviando...'}
                    </>
                  ) : (
                    <>
                      <span className="material-symbols-outlined text-[18px]">send</span>Enviar com
                      PDF via WhatsApp
                    </>
                  )}
                </button>
...
function ReportDetailModal({
  ...
  const { deleteReport, submitForReview, approveReport, isDeleting, isUpdating } =
    useReportMutations();
  const { user } = useUser();
  ...
  const actions: {
  ...
  }[] = [
    {
      ...
      show: ['draft', 'pending_review'].includes(report.status),
    ...
    },
    {
      ...
      show: report.status === 'draft',
      onClick: () =>
        setConfirm({
          action: 'submit',
          title: 'Enviar para Revisão?',
          message: 'O relatório será enviado para revisão.',
          label: 'Enviar',
    ...
    },
    {
      ...
      show: report.status === 'pending_review',
      onClick: () =>
        setConfirm({
          action: 'approve',
          title: 'Aprovar Relatório?',
          message: 'O relatório será aprovado e poderá ser enviado ao responsável.',
          label: 'Aprovar',
    ...
    },
    {
      ...
      label: 'Enviar ao Responsável',
      color: 'text-[#25D366]',
      bg: 'hover:bg-green-50',
      show: ['approved', 'pending_review', 'reviewed'].includes(report.status),
      onClick: () => {
        onClose();
    ...
    },
    {
      ...
      color: 'text-red-500',
      bg: 'hover:bg-red-50',
      show: ['draft', 'archived'].includes(report.status),
      onClick: () =>
        setConfirm({
          action: 'delete',
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl max-w-lg w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-gray-100 shrink-0">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div
                  className={`w-10 h-10 rounded-xl ${typeInfo.color === 'text-[#820AD1]' ? 'bg-[#820AD1]/10' : 'bg-gray-100'} flex items-center justify-center shrink-0`}
                >
                  <span className={`material-symbols-outlined ${typeInfo.color} text-[22px]`}>
                    {typeInfo.icon}
                  </span>
                </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-4 sm:py-5 flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span
                className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${statusInfo.bg} ${statusInfo.text}`}
              >
                <span className={`w-1.5 h-1.5 rounded-full ${statusInfo.dot}`} />
                {statusInfo.label}
              </span>
              <span className="text-xs text-gray-400">
                Paciente: <span className="font-medium text-gray-600">{report.patientName}</span>
              </span>
            </div>
              ...
              <div className="flex flex-col gap-3">
                {sections.map((section, i) => (
                  <div key={i} className="rounded-xl bg-gray-50 border border-gray-100 p-4">
                    {section.title && (
                      <p className="text-[11px] font-bold text-[#820AD1] uppercase tracking-wide mb-2">
                        {section.title}
                      </p>
                    )}
                    <p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">
                      {section.text || <span className="text-gray-400 italic">Sem conteúdo</span>}
                    </p>
                  </div>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <div className="rounded-xl bg-gray-50 border border-gray-100 p-3">
                ...
                <p className="text-sm font-medium text-gray-900 mt-0.5">
                  {formatDateShort(report.updatedAt)}
                </p>
              </div>
              {report.sentAt && (
              ...
              <div className="flex flex-col gap-1 pt-1">
                ...
                {visibleActions.map((a) => (
                  <button
                    key={a.label}
                    onClick={a.onClick}
                    disabled={isBusy || (a.icon === 'picture_as_pdf' && generatingPdf)}
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 flex flex-col gap-5 sm:gap-6">
          {/* Header */}
          ...
          {/* Search + Filters */}
          <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)] gap-3 items-start">
            ...
            <div className="flex flex-col gap-3 xl:min-w-0">
              <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
                {STATUS_TABS.map((tab) => (
                  ...
                  >
                    {tab.dot && (
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${statusFilter === tab.key ? 'bg-white' : tab.dot}`}
                      />
                    )}
                    {tab.icon && (
          ...
          {loading ? (
            <div className="flex items-center justify-center py-16">
              <span className="material-symbols-outlined animate-spin text-[#820AD1] text-3xl">
                progress_activity
              </span>
            </div>
          ) : reports.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {reports.map((report) => {
                ...
                return (
                  <div
                    key={report.id}
                    className="glass-card rounded-xl p-4 flex items-start gap-3 sm:gap-4 hover:shadow-md transition-all cursor-pointer group"
                    onClick={() => setSelectedReport(report)}
                  >
                    <div
                      className={`w-10 h-10 sm:w-11 sm:h-11 rounded-xl ${ti.color === 'text-[#820AD1]' ? 'bg-[#820AD1]/10' : 'bg-gray-100'} flex items-center justify-center shrink-0`}
                    >
                      <span
                        className={`material-symbols-outlined ${ti.color} text-[20px] sm:text-[22px]`}
                      >
                        {ti.icon}
                      </span>
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex sm:hidden flex-wrap items-center gap-2 mt-2">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold ${si.bg} ${si.text}`}
                        >
                          <span className={`w-1.5 h-1.5 rounded-full ${si.dot}`} />
                          {si.label}
                        </span>
                        <span className="text-[11px] text-gray-400 bg-gray-50 px-2 py-1 rounded-full font-medium">
                          {ti.label}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="hidden md:flex items-center gap-1 shrink-0 opacity-60 lg:opacity-0 group-hover:opacity-100 transition-opacity">
                      {['draft', 'pending_review'].includes(report.status) && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/dashboard/relatorios/novo?reportId=${report.id}`);
                          }}
                          className="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-[#820AD1] transition-colors"
                          title="Editar"
                        >
                      ...
                      {['approved', 'pending_review', 'reviewed'].includes(report.status) && (
                        <button
                          ...
                          className="p-2 rounded-lg hover:bg-green-50 text-gray-400 hover:text-[#25D366] transition-colors"
                          title="Enviar ao Responsável"
                        >
                    ...
                    <div className="flex md:hidden items-center gap-1 shrink-0 mt-0.5">
                      {['draft', 'pending_review'].includes(report.status) && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-[#820AD1] transition-colors"
                          title="Editar"
                        >
                      ...
                      {['approved', 'pending_review', 'reviewed'].includes(report.status) && (
                        <button
                          ...
                          className="p-1.5 rounded-lg hover:bg-green-50 text-gray-400 hover:text-[#25D366] transition-colors"
                          title="Enviar ao Responsável"
                        >
