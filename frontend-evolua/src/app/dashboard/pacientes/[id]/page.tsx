        ...
        <div className="glass-panel rounded-2xl p-6 md:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.05)] mb-8 relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-center gap-8 relative z-10">
              ...
              <div>
                <div className="flex items-center justify-center sm:justify-start gap-3 mb-1">
                  ...
                  <Link href={`/dashboard/pacientes/${patientId}/editar`}>
                    <button className="p-1.5 rounded-full text-gray-400 hover:text-[#8A05BE] hover:bg-[#8A05BE]/10 transition-colors" title="Editar Perfil">
                      <span className="material-symbols-outlined text-lg">edit</span>
                    </button>
                  </Link>
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${sb.bg} ${sb.text} ml-1`}>
            ...
            <div className="flex flex-col items-center justify-center gap-2 w-full lg:w-auto border-y lg:border-y-0 lg:border-x border-gray-100 py-4 lg:py-0 lg:px-12">
              <div className="text-center">
                <span className="text-xs uppercase tracking-wider text-gray-400 font-semibold mb-1 block">Diagnóstico Principal</span>
                <span className="text-lg font-bold text-gray-800">{diagnosis}</span>
              </div>
              <div className="w-8 h-px bg-gray-200 lg:hidden" />
              <div className="text-center">
                <span className="text-xs uppercase tracking-wider text-gray-400 font-semibold mb-1 block">Especialidade</span>
                <div className="flex gap-1 flex-wrap justify-center">
                  {specialties.length > 0 ? specialties.map((s, i) => (
                    <span key={i} className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-[#F0E4F9] text-[#8A05BE]">{s}</span>
                  )) : (
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-[#F0E4F9] text-[#8A05BE]">{diagnosis !== "—" ? diagnosis : "—"}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
              <button
                onClick={() => { setWhatsappTemplate("free"); setWhatsappOpen(true) }}
              ...
              <Link href={`/dashboard/pacientes/${patientId}/novo-relatorio`} className="w-full sm:w-auto">
              ...
              <Link href={`/dashboard/agendamentos/novo?patientId=${patientId}`} className="w-full sm:w-auto">
            ...
            <div className="glass-panel rounded-2xl p-6 shadow-[0_4px_30px_rgba(0,0,0,0.05)] h-full">
              ...
              <div className="space-y-5">
                ...
                {patient.guardianName && (
                  <div className="group">
                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">Responsável</span>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-800">
                          {patient.guardianName}{patient.guardianRelationship ? ` (${patient.guardianRelationship})` : ""}
                        </p>
                ...
                {patient.cpf && (
                  <div className="border-t border-gray-100 pt-4">
                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">CPF</span>
                ...
                {patient.address && (
                  <div className="border-t border-gray-100 pt-4">
                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">Endereço</span>
                    <p className="text-sm font-medium text-gray-800">
                    ...
                    <p className="text-xs text-gray-500">
                      {[patient.address.neighborhood, patient.address.city].filter(Boolean).join(", ")}
                    </p>
                  </div>
                )}

                {/* Histórico Rápido */}
                {patient.medicalHistory && (patient.medicalHistory.medications?.length || patient.medicalHistory.allergies?.length) && (
                  <div className="border-t border-gray-100 pt-4">
                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide block mb-1">Histórico Rápido</span>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {patient.medicalHistory.allergies?.map((a, i) => (
                        <span key={`a-${i}`} className="px-2 py-1 bg-yellow-50 text-yellow-700 text-xs rounded-md border border-yellow-100">
                          Alergia: {a}
                        </span>
                      ))}
                      {patient.medicalHistory.medications?.map((m, i) => (
                        <span key={`m-${i}`} className="px-2 py-1 bg-blue-50 text-blue-700 text-xs rounded-md border border-blue-100">
                          {m}
                        </span>
                      ))}
                    </div>
              ...
              <div className="relative pl-2 space-y-0 overflow-y-auto max-h-[600px] pr-2">
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            {new Date(nextApt.dateTime).toLocaleDateString("pt-BR", { weekday: "long", day: "numeric", month: "short" })},{" "}
                            {new Date(nextApt.dateTime).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
                          </p>
                        </div>
                        <span className="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-600 text-xs font-bold uppercase tracking-wider">Futura</span>
                      </div>
                      <p className="text-sm text-gray-600 mb-3">Foco: {nextApt.type || "Sessão terapêutica"}</p>
                      <button className="text-sm text-[#8A05BE] font-medium hover:underline flex items-center gap-1">
                        Ver detalhes do planejamento <span className="material-symbols-outlined text-sm">arrow_forward</span>
                      </button>
                    </div>
                  </div>
                )}
                ...
                {completed.slice(0, 3).map((apt, idx) => (
                  <div key={apt.id} className="relative pl-8 pb-8">
                    <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gray-200 ml-[1.15rem]" />
                    <div className={`absolute left-0 top-0 mt-1.5 w-10 h-10 rounded-full flex items-center justify-center z-10 ${
                      idx === 0 ? "bg-[#F0E4F9]" : "bg-gray-100"
                    }`}>
                      <span className={`material-symbols-outlined text-sm ${idx === 0 ? "text-[#8A05BE]" : "text-gray-400"}`}>check</span>
                    </div>
                    <div className="bg-white/60 border border-gray-100 p-4 rounded-xl hover:bg-white transition-colors">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            {new Date(apt.dateTime).toLocaleDateString("pt-BR", { day: "numeric", month: "short", year: "numeric" })},{" "}
                        ...
                        <span className="px-2.5 py-1 rounded-lg bg-green-50 text-green-600 text-xs font-bold uppercase tracking-wider flex items-center gap-1">
                          <span className="material-symbols-outlined text-[10px]">description</span> Pronto
                        </span>
                      </div>
                      <p className="text-sm text-gray-600 line-clamp-2">
                        {apt.type ? `${apt.type} — ` : ""}{apt.duration} min
                      </p>
                    </div>
                  </div>
                ))}
                ...
                {/* Reports in timeline */}
                {reports.filter(r => r.status === "draft" || r.status === "pending_review").slice(0, 2).map((report) => (
                  <div key={report.id} className="relative pl-8 pb-8">
                    ...
                    <div className="bg-white/60 border border-gray-100 p-4 rounded-xl hover:bg-white transition-colors">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h4 className="font-bold text-gray-900">{report.title || "Relatório"}</h4>
                          <p className="text-sm text-gray-500">{formatReportDate(report.createdAt)}</p>
                        </div>
                        <span className="px-2.5 py-1 rounded-lg bg-yellow-50 text-yellow-600 text-xs font-bold uppercase tracking-wider flex items-center gap-1">
                          <span className="material-symbols-outlined text-[10px]">edit</span> Pendente
                        </span>
                      </div>
                      <p className="text-sm text-gray-600">{formatReportType(report.type)}</p>
                      <div className="mt-3">
                        <Link href={`/dashboard/pacientes/${patientId}/revisar-relatorio?reportId=${report.id}`}>
                ...
                {completed.length === 0 && !nextApt && reports.length === 0 && (
                  <div className="text-center py-8">
                    <span className="material-symbols-outlined text-4xl text-gray-300 mb-2">timeline</span>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="glass-panel rounded-2xl p-6 shadow-[0_4px_30px_rgba(0,0,0,0.05)]">
              <div className="flex justify-between items-center mb-5">
                <h3 className="font-bold text-gray-900 text-sm uppercase tracking-wide">Plano Terapêutico</h3>
                <Link href={`/dashboard/pacientes/${patientId}/planos-metas`}>
              ...
              {/* Progress bars */}
              <div className="space-y-5">
                <ProgressBar label="Interação Social" value={70} />
                <ProgressBar label="Compreensão Verbal" value={45} />
                <ProgressBar label="Vocabulário" value={60} />
              </div>

              {/* Metas Clínicas */}
              <div className="mt-6 pt-5 border-t border-gray-100">
                <h4 className="font-bold text-gray-900 text-xs uppercase tracking-wide mb-3">Metas Clínicas</h4>
                <div className="space-y-3">
                  <GoalCheckbox label="Produção do fonema /s/ em início de palavra" checked={false} />
                  <GoalCheckbox label="Manter contato visual por 5 segundos" checked={false} />
                  <GoalCheckbox label="Identificar 5 cores básicas" checked={true} />
                </div>
              </div>
            </div>
            ...
            <div className="glass-panel rounded-2xl p-6 shadow-[0_4px_30px_rgba(0,0,0,0.05)]">
              <div className="flex justify-between items-center mb-5">
                <h3 className="font-bold text-gray-900 text-sm uppercase tracking-wide">Documentos</h3>
              ...
              <div className="space-y-3">
                {reportsLoading ? (
                  ...
                  reports.slice(0, 3).map((report) => {
                    ...
                    const ic = icMap[report.type] || { icon: "folder", bg: "bg-purple-50", color: "text-[#8A05BE]" }
                    return (
                      <button
                        key={report.id}
                      ...
                      >
                        <div className={`w-10 h-10 rounded-lg ${ic.bg} flex items-center justify-center ${ic.color}`}>
                          <span className="material-symbols-outlined">{ic.icon}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-800 truncate">{report.title || "Relatório"}</p>
                          <p className="text-xs text-gray-400">{formatReportType(report.type)} • {formatReportDate(report.createdAt)}</p>
                        </div>
                        <span className="material-symbols-outlined text-gray-300 group-hover:text-[#8A05BE] transition-colors">download</span>
                      </button>
                    )
                  })
                )}
            ...
            {/* Ações */}
            <div className="glass-panel rounded-2xl p-6 shadow-[0_4px_30px_rgba(0,0,0,0.05)]">
              <h3 className="font-bold text-gray-900 text-sm uppercase tracking-wide mb-4">Ações</h3>
              <div className="flex flex-col gap-2">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={handleDischarge} className="bg-[#8A05BE] hover:bg-[#8A05BE]/90">Confirmar</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                )}
                {patient.status === "discharged" && (
                  <button onClick={handleReactivate} disabled={mutationLoading} className="w-full flex items-center gap-3 p-3 rounded-xl bg-white border border-gray-100 hover:border-green-300 hover:shadow-md transition-all text-left group">
                    ...
                    <span className="text-sm font-bold text-gray-900 group-hover:text-green-700 transition-colors">Reativar</span>
                  </button>
                )}
                <AlertDialog>
                  <AlertDialogTrigger asChild>
  ...
  return (
    <div className="flex items-start gap-3 group">
      <label className="flex items-start gap-3 cursor-pointer">
        ...
        />
        <span className={`text-sm text-gray-600 group-hover:text-[#8A05BE] transition-colors leading-snug ${isChecked ? "line-through opacity-70" : ""}`}>
          {label}
        </span>
      </label>
    </div>
