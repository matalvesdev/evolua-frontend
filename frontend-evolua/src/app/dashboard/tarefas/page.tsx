      ...
      <div className="bg-white rounded-2xl p-6 max-w-sm w-full flex flex-col gap-4 shadow-2xl">
        ...
        <div className="flex gap-3 justify-end pt-2">
          ...
          <button
            onClick={onConfirm}
            disabled={loading}
            className="px-4 py-2 rounded-full text-sm font-bold text-white bg-red-500 hover:bg-red-600 transition-all shadow-lg disabled:opacity-50"
          >
            {confirmLabel}
          </button>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl sm:rounded-3xl max-w-lg w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="px-5 sm:px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#820AD1]/10 flex items-center justify-center">
              <span className="material-symbols-outlined text-[#820AD1] text-[22px]">
                {task ? 'edit' : 'add_task'}
              </span>
            </div>
            <div>
              ...
              <p className="text-xs text-gray-500">
                {task ? 'Atualize as informações' : 'Preencha os campos abaixo'}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="Ex: Ligar para responsável do João"
        ...
        <div className="px-5 sm:px-6 py-4 border-t border-gray-100 flex flex-col-reverse sm:flex-row gap-3 shrink-0">
          <button
            onClick={onClose}
            className="flex-1 h-11 rounded-xl border border-gray-200 text-gray-700 hover:bg-gray-50 font-bold text-sm transition-all"
          >
            Cancelar
          </button>
          <button
...
function TaskFormModal({
  ...
  if (!open) return null;
  return (
    <TaskFormModalInner key={task?.id || 'new'} onClose={onClose} task={task} patients={patients} />
  );
}
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 flex flex-col gap-5 sm:gap-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
          {[
            ...
            <div key={stat.label} className="glass-card rounded-xl p-4 flex items-center gap-3">
              ...
              >
                <span className={`material-symbols-outlined ${stat.iconColor} text-[20px]`}>
                  {stat.icon}
                </span>
              </div>
              <div>
        ...
        <p className="text-xs text-gray-400">
          {totalFiltered} tarefa{totalFiltered !== 1 ? 's' : ''} encontrada
          {totalFiltered !== 1 ? 's' : ''}
        </p>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-xs text-gray-400">
              {tasks.length === 0 ? 'Crie sua primeira tarefa' : 'Ajuste os filtros'}
            </p>
          </div>
        ) : (
                ...
                >
                  ...
                  <button
                    ...
                    className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center shrink-0 mt-0.5 transition-all ${
                      completed
                        ...
                        : 'border-gray-300 hover:border-[#820AD1]/50'
                    }`}
                  >
                    {completed && (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <p
                        className={`text-sm font-bold text-gray-900 pr-2 ${completed ? 'line-through text-gray-500' : ''}`}
                      >
                        {task.title}
                      </p>
                      {/* Actions */}
                      <div className="hidden sm:flex items-center gap-1 shrink-0 opacity-60 md:opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                      {/* Priority */}
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${pi.bg} ${pi.color}`}
                      >
                      ...
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-gray-100 text-gray-600">
                        <span className={`material-symbols-outlined ${ti.color} text-[12px]`}>
                          {ti.icon}
                        </span>
                        {ti.label}
                      </span>
                      ...
                      {task.dueDate && (
                        ...
                        >
                          <span className="material-symbols-outlined text-[14px]">
                            {overdue ? 'warning' : today ? 'today' : 'schedule'}
                          </span>
                          {overdue
                            ? `Atrasada · ${formatDateShort(task.dueDate)}`
                            : today
                              ? 'Hoje'
                    ...
                    <div className="flex sm:hidden items-center justify-end gap-1 mt-3">
                      <button
                        onClick={() => handleEdit(task)}
                        className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-[#820AD1] transition-colors"
                      >
