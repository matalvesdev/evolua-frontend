...
export default function ObjetivosPage() {
  ...
  const handlePriorityChange = (id: string, checked: boolean) => {
    setPriorities((prev) => (checked ? [...prev, id] : prev.filter((p) => p !== id)));
  };
  ...
  const handleContinue = () => {
    ...
    localStorage.setItem(
      'onboarding_data',
      JSON.stringify({ ...existing, challenge, priorities, additionalDetails })
    );
  ...
  return (
    ...
    >
      ...
      <GlassCard className="text-left items-stretch gap-8">
        {/* Challenge Selection */}
        <OnboardingFormField label="Maior desafio atual" icon={<AlertCircle className="size-4" />}>
        ...
        {/* Priority Selection */}
        <OnboardingFormField label="Prioridade com a Evolua" icon={<Star className="size-4" />}>
          <div className="flex flex-wrap gap-2 mt-2">
            {PRIORITY_OPTIONS.map((option) => (
              <OnboardingChipCheckbox
                ...
                onChange={(checked) => handlePriorityChange(option.id, checked)}
              />
            ))}
          </div>
        </OnboardingFormField>
