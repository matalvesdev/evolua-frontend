-- =============================================================================
-- 20260509000001_blog_posts.sql
-- Tabela e seed do blog público da landing.
-- Lida pela anon key — RLS permite SELECT para qualquer um. Escrita só via
-- service_role (admin) ou painel do Supabase.
-- =============================================================================
...
create table if not exists public.blog_posts (
  ...
  categoria     text not null check (categoria in ('Marketing','Gestão','Clínica','Carreira','Tecnologia')),
  autor         text not null default 'Equipe Evolua',
  data          date not null default current_date,
  tempo_leitura int  not null default 5 check (tempo_leitura > 0),
...
create index if not exists blog_posts_data_idx       on public.blog_posts (data desc);
create index if not exists blog_posts_categoria_idx  on public.blog_posts (categoria);
create index if not exists blog_posts_destaque_idx   on public.blog_posts (destaque) where destaque = true;

alter table public.blog_posts enable row level security;

-- Leitura pública (anon e authenticated). Sem política de escrita: apenas
-- service_role (BYPASSRLS) consegue inserir/atualizar/deletar.
...
-- =============================================================================
-- Seed: 6 posts iniciais (idempotente via ON CONFLICT)
-- =============================================================================

insert into public.blog_posts (slug, titulo, subtitulo, categoria, autor, data, tempo_leitura, destaque, imagem, corpo) values
(
